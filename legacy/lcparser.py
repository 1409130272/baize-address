...
def china_location_loader(detail=False):
    ...
    """
    with open(os.path.join(DIR_PATH, 'china_location.txt'),
...
def china_location_change_loader():
    ...
    location_change_jio = read_file_by_line(
        os.path.join(DIR_PATH, 'china_location_change.txt'),
    ...
    for line in location_change_jio:
        ...
        #   新增功能：旧地址改为新地址后使用新地址相关联的行政区域代码
        admin_code = line_seg[2]
        ...
        if len(orig_line_seg) == 8:  # 县一级
            location_change_dict.update(
                {'date': orig_line_seg[0], 'department': orig_line_seg[1],
                 ...
                 'new_loc': new_line_seg, 'code':admin_code})
            ...
            location_change_dict.update(
                {'date': orig_line_seg[0], 'department': orig_line_seg[1],
                 ...
                 'new_loc': [new_line_seg[0], new_line_seg[1], None], 'code':admin_code})
...
class LocationParser(object):
    ...
    def _mapping(self, china_loc, china_change_loc):
        ...
        # 不再使用固有的000000行政区域码，使用新地区的code
        for item in china_change_loc:
            self.administrative_map_list.append(
                [item['code'], item['old_loc'][0], item['old_loc'][1],
    ...
    def _get_final_res(self, final_admin, location_text, county_dup_list,
        ...
        final_city = None
        final_county = None
        final_area_code = final_admin[0] if final_admin[0] else None
